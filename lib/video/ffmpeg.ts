...
export async function createHlsAndThumbnail(inputPath: string, outputDir: string): Promise<HlsResult> {
  ...
  const rendition480 = path.join(hlsDir, "480p");
  const rendition720 = path.join(hlsDir, "720p");
  await fs.mkdir(rendition360, { recursive: true });
  await fs.mkdir(rendition480, { recursive: true });
  ...
  const args = [
    ...
    inputPath,
    "-filter_complex",
    "[0:v]split=3[v1][v2][v3]; [v1]scale=w=640:h=360:force_original_aspect_ratio=decrease[v1out]; [v2]scale=w=854:h=480:force_original_aspect_ratio=decrease[v2out]; [v3]scale=w=1280:h=720:force_original_aspect_ratio=decrease[v3out]",
    "-map",
    "[v1out]",
    "-map",
    "0:a?",
    ...
    "128k",
    "-hls_segment_filename",
    path.join(rendition480, "seg_%03d.ts"),
    path.join(rendition480, "index.m3u8"),
    "-map",
    "[v3out]",
    "-map",
    "0:a?",
